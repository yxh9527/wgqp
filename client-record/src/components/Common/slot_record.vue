    ...
    <el-row>
      <el-col :span="24">
        <el-table
          ...
          class="slot-record-header J_Record"
        >
          <el-table-column
            type="index"
...
<style lang="scss">
.slot-record-header .el-table__header-wrapper {
  position: absolute;
  z-index: 100;
}
.slot-record-header .el-table__body-wrapper {
  margin-top: 48px;
}
.el-table__body tr {
  height: 40px;
...
export default {
  ...
  methods: {
    ...
    calcSum() {
      if (this.table_data_list) {
        for (var key in this.table_data_list) {
          let sum = (parseFloat(this.table_data_list[key].all_bonus) + parseFloat(this.table_data_list[key].jp_bonus)).toFixed(2)
          this.table_data_list[key].all_bonus = sum == 0 ? 0 : sum
        }
      }
    },
    indexMethod(index) {
    ...
    queryRowDetail(row) {
      Lockr.set('slot_detail', row)
      bus.$emit('slot_detail', row)
    },
    getRecordData(form) {
      const form_data = {
        params: {
      ...
      this.apiGet(_g.apiUrl() + 'slot_log', form_data).then(res => {
