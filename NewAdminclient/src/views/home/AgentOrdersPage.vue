...
import { getAgentOrderStats } from "@/api/data";
import { calcKill, safeNumber, toFixedValue } from "./homeHelpers";

export default {
  name: "AgentOrdersPage",
  components: {
    AppTable,
  },
  data() {
  ...
  computed: {
    columns() {
      return [
        { title: "日期", key: "day", align: "center", minWidth: 140 },
  ...
  methods: {
    async fetchGameList() {
      this.loading = true;
      try {
        const response = await getAgentOrderStats({
          page: this.page,
        ...
        this.total = safeNumber(payload.total);
        this.pageSize = safeNumber(payload.page_size || this.pageSize);
      } finally {
        this.loading = false;
      }
    },
