...
func InitZapLogger(path, prefix string, ip string) {
	currentPath := ""
	if path == "" {
	...
	hook := lumberjack.Logger{
		Filename:   filepath.Join(currentPath, "logs/debug", "data-center.log"), // 日志文件路径
	...
	err_hook := lumberjack.Logger{
		Filename:   filepath.Join(currentPath, "logs/error", "data-center.log"), // 日志文件路径
		MaxSize:    128,                                                         // 最大日志大小（Mb级别）
		MaxBackups: 30,                                                          // 最多保留30个备份
		MaxAge:     7,                                                           // days
		Compress:   true,                                                        // 是否压缩 disabled by default
