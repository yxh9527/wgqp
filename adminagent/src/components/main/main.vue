    ...
    <Layout>
      ...
      <Content class="main-content-con">
        <Layout class="main-layout-con" style="overflow-x: auto">
          <Content class="content-wrapper" style="min-width: 1280px">
  ...
  mounted() {
    ...
    if (ResizeObserver) {
      ...
      const resizeObserver = new ResizeObserver((entries) => {
        //execute marquee width
        if (document.querySelector(".ivu-layout-header")) {
          if (!this.$refs.marqueeMsg) {
            return;
          }
          this.$refs.marqueeMsg.style = `width:${
    ...
    this.$nextTick(() => {
      getUnreadCount().then((res) => {
        this.$store.commit(
          "setMessageCount",
          res.data.data.filter((x) => !x.isRead).length
        );
        setTimeout(() => {
          res.data.data.map((item) => {
            if (!item.isRead) {
              this.$refs.marqueeMsg.innerHTML = item.title;
            }
          });
        }, 20);
      });
