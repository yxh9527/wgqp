    ...
    <div>
        ...
        <Menu width="auto" active-name :class="titleClass" @on-select="handleView">
          <MenuItem v-for="item in messageList" :name="item.msg_id" :key="`msg_${item.msg_id}`">
            <div>
              ...
              <Badge status="default" :text="item.create_time+ ' ' + (item.sendName ? (item.sendName.length>6 ? item.sendName.substr(0,6)+'...':item.sendName) : '')" />
              <Button
      ...
      <div class="message-page-con message-view-con" style="left: 520px;">
        ...
        <div class="message-view-header">
          ...
          <time class="message-view-time">{{ showingMsgItem.create_time }} {{showingMsgItem.sendName ? (showingMsgItem.sendName.length>6 ? showingMsgItem.sendName.substr(0,6)+'...':showingMsgItem.sendName):''}}</time>
        </div>
