...
const (
	...
	LotteryService_RenewReservation_FullMethodName       = "/lottery.LotteryService/RenewReservation"
...
type LotteryServiceClient interface {
	...
	// 延长已成功的赔付预留有效期；只改 expiresAt，不改金额/结果/奖池占用。
	// 禁止用 PrePay 做续期。仅 RESERVED 可续；超时由资金侧自动 EXPIRED 并回退水池。
	RenewReservation(ctx context.Context, in *RenewReservationRequest, opts ...grpc.CallOption) (*RenewReservationResponse, error)
...
func (c *lotteryServiceClient) RenewReservation(ctx context.Context, in *RenewReservationRequest, opts ...grpc.CallOption) (*RenewReservationResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(RenewReservationResponse)
	err := c.cc.Invoke(ctx, LotteryService_RenewReservation_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type LotteryServiceServer interface {
	...
	// 延长已成功的赔付预留有效期；只改 expiresAt，不改金额/结果/奖池占用。
	// 禁止用 PrePay 做续期。仅 RESERVED 可续；超时由资金侧自动 EXPIRED 并回退水池。
	RenewReservation(context.Context, *RenewReservationRequest) (*RenewReservationResponse, error)
	// 批量结算本局全部真人玩家，或按 VOID_REFUND 原路退回全部下注。
...
func (UnimplementedLotteryServiceServer) RenewReservation(context.Context, *RenewReservationRequest) (*RenewReservationResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method RenewReservation not implemented")
}
...
func _LotteryService_RenewReservation_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(RenewReservationRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(LotteryServiceServer).RenewReservation(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: LotteryService_RenewReservation_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(LotteryServiceServer).RenewReservation(ctx, req.(*RenewReservationRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var LotteryService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "RenewReservation",
			Handler:    _LotteryService_RenewReservation_Handler,
		},
		{
