...
> 文档基线：2026-08-17 当前工作区代码（安全向补丁见文末链接）。本文描述已经实现的行为，不代表未来设计目标。
...
6. 自动回收超时预留；支持房间心跳 `RenewReservation` 续期（禁止用 PrePay 续期）。
...
| `Bet` | 扣除玩家余额并记录本局下注 |
| `PrePay` | 建立或调整赔付预留 |
| `RenewReservation` | 仅延长已 RESERVED 预留的 `expiresAt` |
| `Settlement` | 正常结算或整局退款 |
| `CancelBet` | 取消本局某玩家 ACTIVE 投注 |
| `GetRoundFinanceState` | 查询牌局资金状态 |
| `GetPoolControlDecision` | 只读水池控制决策 |

预留占用由结算、整局退款或超时回收释放；续期只改过期时间，不改金额与结果。

正常路径会计与崩溃/重试下的**严格保证边界**见：[资金预留与结算安全保证.md](./资金预留与结算安全保证.md)。
